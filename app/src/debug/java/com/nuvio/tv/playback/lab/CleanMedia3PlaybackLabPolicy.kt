...
internal object CleanPlaybackSmokeLine {
    ...
    fun session(generation: Long): String =
        "$PREFIX event=SESSION engine=MEDIA3 profile=GUIDE generation=$generation"
    ...
    fun state(
        ...
        "$PREFIX event=STATE engine=MEDIA3 profile=GUIDE generation=$generation " +
            "player_state=${state.name} play_when_ready=$playWhenReady " +
            "is_loading=$loading"

    fun renderer(decoderName: String? = null, sampleMimeType: String? = null): String = buildString {
        append("$PREFIX event=RENDERER engine=MEDIA3 renderer=MediaCodecVideoRenderer")
    ...
    fun surface(mode: SurfaceMode, valid: Boolean, width: Int, height: Int): String {
        require(mode == SurfaceMode.SURFACE_VIEW || mode == SurfaceMode.TEXTURE_VIEW)
        return "$PREFIX event=SURFACE engine=MEDIA3 surface_type=${mode.name} " +
            "surface_valid=$valid surface_width=${width.coerceAtLeast(0)} " +
            "surface_height=${height.coerceAtLeast(0)} secure=false"
    }

    fun firstFrame(): String =
        "$PREFIX event=VIDEO engine=MEDIA3 rendered_first_frame=true"

    fun videoSize(width: Int, height: Int): String {
        require(width > 0 && height > 0)
        return "$PREFIX event=VIDEO engine=MEDIA3 video_width=$width video_height=$height"
    }

    fun metrics(rendered: Long?, dropped: Long?): String =
        "$PREFIX event=VIDEO engine=MEDIA3 rendered_first_frame=${(rendered ?: 0) > 0} " +
            "dropped_frames=${(dropped ?: 0).coerceIn(0, Int.MAX_VALUE.toLong())}"

    fun error(failure: PlaybackFailure): String =
        "$PREFIX event=ERROR engine=MEDIA3 error_domain=${failure.smokeDomain()} " +
            "error_code=${failure.code.name} phase=${failure.phase.smokePhase()} " +
            "fatal=${failure.retryability == Retryability.FATAL}"

    fun release(nonce: String, hardAbort: Boolean): String {
        require(RELEASE_NONCE.matches(nonce))
        return "$PREFIX event=RELEASE engine=MEDIA3 " +
